...
We will see a quick setup to configure the ``archivebox`` formula for different DNS domain names and several roles.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/archivebox/parameters/dns:domain/
    ln -s dns:domain /srv/salt/archivebox/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/archivebox/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/archivebox/parameters/dns:domain/example.com.yaml.jinja``:
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/archivebox/parameters/roles
...
We create a configuration for the role ``archivebox/server`` in ``/srv/salt/archivebox/parameters/roles/archivebox/server.yaml``:
...
We create another configuration for the role ``archivebox/client`` in ``/srv/salt/archivebox/parameters/roles/archivebox/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles archivebox/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles archivebox/client
...
.. code-block:: console
    ...
    mv /srv/salt/archivebox/parameters/dns:domain/  '/srv/salt/archivebox/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-archivebox-as-json:
      test.nop:
        - name: {{ archivebox | json }}
    ...
    archivebox-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='archivebox-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ archivebox.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``archivebox`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ archivebox['winner'] }}
